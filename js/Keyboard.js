...
export default class Keyboard {
  ...
  init(langCode) {
    ...
    this.KeyBase = language[this.langCode];
    this.output = create(
      'textarea',
      'output',
      null,
  ...
  handleEvent = (e) => {
    ...
    if (type.match(/keydown|mousedown/)) {
      ...
      if (code.match(/Shif/)) this.shiftKey = true;
  ...
  switchLanguage = () => {
    ...
    // let langIndex = langAbbr.indexOf(this.langCode);
    let langIndex = langAbbr.indexOf(this.container.dataset.language);
    this.keyBase =
      langIndex + 1 < langAbbr.length
    ...
    if (isTrue) {
      this.keyButtons.forEach((button) => {
        if (button.sub) {
          if (this.shiftKey) {
            ...
            button.letter.classList.add('inactive');
          }
        }
        if (
          button.isFnKey &&
      ...
      this.keyButtons.forEach((button) => {
        if (button.sub.innerHTML && !button.isFnKey) {
          ...
          button.letter.classList.remove('inactive');
  ...
  printToOutput(keyObj, symbol) {
    ...
    const fnButtonHandler = {
      ...
      ArrowUp: () => {
        const positionFormLeft = this.output.value
          .slice(0, cursorPos)
          .match(/(\n).*$(?!\1)/g) || [[1]];
        cursorPos -= positionFormLeft[0].length;
      },
      ArrowDown: () => {
        const positionFormLeft = this.output.value
          .slice(cursorPos)
